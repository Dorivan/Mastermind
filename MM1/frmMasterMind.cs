...
namespace MasterMindWin
{
	public class frmMasterMind : System.Windows.Forms.Form
	{
		...
		protected override void OnMouseUp(MouseEventArgs e)//33
		{
			PegPicking = false;
		...
		private void mnuOptions_Click(object sender, System.EventArgs e)    //меню программы 33
		{
			...
			ColorPellete = GetPelleteRectangles(
				(byte)(myBoard.pegs.Values.Count),
		...
		private void CreateOffScreenImage()//формирвоание игрового поля
		{
			...
			gOffScreen.DrawLine(DarkPen,StartPoint.X + 4*boxWidth + border,StartPoint.Y,StartPoint.X + 4*boxWidth + border,StartPoint.Y+ mychois* boxWidth);
            ...
            for (int i=0;i< mychois; i++)//!!!
			{
				gOffScreen.DrawLine(DarkPen,Boxes[1,0].X,Boxes[i,0].Y ,Boxes[1,0].X + Boxes[1,0].Width*5,Boxes[i,0].Y );
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        {
                            gOffScreen.DrawImage(this.PegImages[myBoard.Rowss[i][j] - 1], Boxes[i, j].X + 3, Boxes[i, j].Y + 3);
                        }
                        else
                        {
        ...
        public Board tre (object sender, EventArgs e)
        {
